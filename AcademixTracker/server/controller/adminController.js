...
// export const adminLogin = async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      ...
      "sEcReT", // Ensure this matches your auth middleware
      { expiresIn: "1h" }
    );
    ...
    console.log("LOGIN ERROR:", error); // Print error to terminal if it fails
...
//   try {
//     console.log("🟢 1. I AM WORKING! Data received:", req.body);
...
export const addAdmin = async (req, res) => {
  try {
    ...
    // 1. EXTRACT 'password' correctly
    const { name, dob, department, contactNumber, avatar, email, joiningYear, password } = req.body;
    ...
    if (!existingDepartment) {
      return res.status(400).json({ message: "Department not found. Please create it in DB first." });
    }

    let departmentHelper = existingDepartment.departmentCode;
    const admins = await Admin.find({ department });
    let helper;
    if (admins.length < 10) helper = "00" + admins.length.toString();
    else if (admins.length < 100 && admins.length > 9) helper = "0" + admins.length.toString();
    else helper = admins.length.toString();

    var date = new Date();
    var components = ["ADM", date.getFullYear(), departmentHelper, helper];
    var username = components.join("");

    // 4. CRITICAL FIX: Hash the TYPED password, NOT the DOB
    const hashedPassword = await bcrypt.hash(password, 10); 
    
    const newAdmin = await new Admin({
      name, email, password: hashedPassword, joiningYear, username, department, avatar, contactNumber, dob,
...
export const addFaculty = async (req, res) => {
  try {
    ...
    // 1. EXTRACT DATA (Includes 'password')
    const { name, dob, department, contactNumber, avatar, email, joiningYear, gender, designation, password } = req.body;
    ...
    const existingDepartment = await Department.findOne({ department });
    if (!existingDepartment) {
      console.log("❌ ERROR: Department '" + department + "' not found");
    ...
    const faculties = await Faculty.find({ department });
    let helper;
    if (faculties.length < 10) { helper = "00" + faculties.length.toString(); }
    else if (faculties.length < 100 && faculties.length > 9) { helper = "0" + faculties.length.toString(); }
    else { helper = faculties.length.toString(); }

    var date = new Date();
    var components = ["FAC", date.getFullYear(), departmentHelper, helper];
    var username = components.join("");

    // 5. ENCRYPT PASSWORD (Uses the typed password, NOT DOB)
    const hashedPassword = await bcrypt.hash(password, 10);
    var passwordUpdated = false;

    // 6. SAVE
    const newFaculty = await new Faculty({
      name, email, password: hashedPassword, joiningYear, username, department, avatar, contactNumber, dob, gender, designation, passwordUpdated,
    });

    await newFaculty.save();
    console.log("✅ SUCCESS! Faculty Created. Username:", username);
